...
RESULTS_PKL = 'part-2a.pkl'
MASTER_IP = '172.18.0.2'
NODE_RANK = None
LOG_FILE = 'logs.log'

### RUN WITH: `python part2_main.py -l 0` ###

def average_gradients(model, rank):
    # TODO
    for p in model.parameters():
        if rank == 0:
            inputs = [torch.zeros(p.grad.size()) for _ in range(dist.get_world_size())]
            dist.gather(p.grad, gather_list=inputs,dst=0)
...
#########################################
        ...
        # print(f'Num of samples ({batch_idx}): {len(data)}')
        ...
        average_gradients(model, NODE_RANK)
